...
use crate::msg::{BirthdayResponse, InstantiateMsg, ExecuteMsg, QueryMsg, ScoreResponse, MigrateMsg};
...
pub fn instantiate(
  ...
  Ok(Response::new()
    ...
    .add_attribute("birthday", msg.birthday.to_string())
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
  match msg {
    QueryMsg::GetBirthday {} => to_binary(&query_birthday(deps)?),
    QueryMsg::GetScores {} => to_binary(&query_scores(deps)?),
  }
}

fn query_birthday(deps: Deps) -> StdResult<BirthdayResponse> {
  let state = STORAGE.load(deps.storage)?;
  Ok(BirthdayResponse { birthday: state.birthday })
}
